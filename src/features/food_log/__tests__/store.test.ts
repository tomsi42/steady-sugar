...
const mockGetAll = jest.fn<FoodEntry[], []>();
const mockInsert = jest.fn<FoodEntry, [any]>();
const mockUpdate = jest.fn<FoodEntry, [number, any]>();
...
describe('useFoodLogStore', () => {
  ...
  describe('update', () => {
    it('updates an entry in state', () => {
      ...
      useFoodLogStore.getState().update(1, { name: 'Avocado Toast' });
  ...
  describe('remove', () => {
    it('deletes an entry and removes it from state', () => {
      ...
      useFoodLogStore.getState().remove(1);
  ...
  describe('restore', () => {
    it('re-inserts a deleted entry and sorts by timestamp descending', () => {
      ...
      useFoodLogStore.getState().restore({
        name: 'Oatmeal',
        category: 'breakfast',
